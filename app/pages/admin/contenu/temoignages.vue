...
const openAddModal = () => {
  editingTestimonial.value = {
    ...
    quote: '',
    embedUrl: '',
    order: testimonials.value.length + 1,
  }
...
const saveTestimonial = async () => {
  loading.value = true
  try {
    // TODO: API call to save testimonial
    await new Promise(resolve => setTimeout(resolve, 1000))

    if (editingTestimonial.value.id) {
      // Update existing
      const index = testimonials.value.findIndex(t => t.id === editingTestimonial.value.id)
      if (index !== -1) {
        testimonials.value[index] = { ...editingTestimonial.value }
      }
    } else {
      // Add new
      testimonials.value.push({
        ...editingTestimonial.value,
        id: Math.max(...testimonials.value.map(t => t.id)) + 1,
      })
    }
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-gray-600 dark:text-gray-400 mt-2">
          Gérer les témoignages vidéo de vos clients
        </p>
      </div>
      <UButton
        color="primary"
        ...
        <div class="p-6 space-y-6">
          ...
          <div v-if="editingTestimonial" class="space-y-4">
            ...
            <UFormField label="Citation" required>
              <UTextarea
                v-model="editingTestimonial.quote"
                :rows="4"
            ...
            <UFormField label="URL d'embed YouTube" required>
              <UInput
                v-model="editingTestimonial.embedUrl"
                size="lg"
                placeholder="https://www.youtube.com/embed/..." />
              <template #hint>
                <span class="text-xs text-gray-500">
                  Utilisez le format: https://www.youtube.com/embed/VIDEO_ID
                </span>
              </template>
            </UFormField>
